  ...
  return async function (createFFmpegCore = {}) {
    ...
    function ___syscall__newselect(
      ...
      try {
        ...
        const srcReadHigh = readfds ? HEAP32[(readfds + 4) >> 2] : 0
        const srcWriteLow = writefds ? HEAP32[writefds >> 2] : 0
        const srcWriteHigh = writefds ? HEAP32[(writefds + 4) >> 2] : 0
        const srcExceptLow = exceptfds ? HEAP32[exceptfds >> 2] : 0
        const srcExceptHigh = exceptfds ? HEAP32[(exceptfds + 4) >> 2] : 0
        let dstReadLow = 0
        let dstReadHigh = 0
        let dstWriteLow = 0
        let dstWriteHigh = 0
    ...
    function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
      try {
        const sock = getSocketFromFD(fd)
        const dest = getSocketAddress(addr, addr_len, true)
        if (!dest) {
    ...
    const MAX_INT53 = 9007199254740992
    const MIN_INT53 = -9007199254740992
    ...
    function _getnameinfo(sa, salen, node, nodelen, serv, servlen, flags) {
      const info = readSockaddr(sa, salen)
      if (info.errno) {
        return -6
      }
      let port = info.port
    ...
    const _ff_h264_cabac_tables = (Module._ff_h264_cabac_tables = 1529068)
    const ___start_em_js = (Module.___start_em_js = 6024597)
    const ___stop_em_js = (Module.___stop_em_js = 6024774)
